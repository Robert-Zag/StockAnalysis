...
def plot_portfolio(portfolio):
    ...
    sp_price.plot(ax=axs[0], label='Cena Indexu S&P 500', lw=1)
    portfolio_val = (portfolio['total_portfolio_value'] /
                     portfolio['total_portfolio_value'][0] * 100)
    portfolio_val.plot(ax=axs[0], label='Hodnota Portfólia', lw=1)
    ...
    axs[0].set_ylabel('Hodnota (%)')
    axs[1].set_ylabel('Strata Hodnoty (%)')
    axs[1].set_xlabel('')
    axs[0].set_title('Vývoj hodnoty portfólia oproti referenčnému indexu')
...
def plot_df(df, symbol, name, strategy, params, save_png=False, show_plot=False):
    ...
    for ax in axs:
        ax.tick_params(axis='x', which='minor', bottom=False)
    df['Adj Close'].plot(ax=axs[0], lw=1, label='Cena', zorder=3, c='orange')

    # plotting the volume as a bar chart
    axs[1].set_ylabel('Objem ($M)')
    ...
    axs[1].fill_between(df.index, 0, (df['Volume']/1_000_000).max(),
                        ...
                        alpha=0.15, label='Prítomnosť v rozhodovacom priestore')
    ...
    if strategy == 'sma':
        df['ma_fast'].plot(ax=axs[0], lw=1, label='Rýchly kĺzavý priemer', c='blue')
        df['ma_slow'].plot(ax=axs[0], lw=1, label='Pomalý kĺzavý priemer', c='green')

    if strategy == 'rsi_sma':
        df['rsi'].plot(ax=axs[2], lw=1, label='RSI', c='orange')
        axs[2].set_ylabel('RSI')
        df['ma'].plot(ax=axs[0], lw=1, label='Kĺzavý priemer')
    ...
    axs[0].scatter(buy_dates, buy_prices, marker='^', s=100, c='#3CAEA3',
                   zorder=4, edgecolor='black', label='Nákup', lw=0.6)
    ...
    axs[0].scatter(sell_dates, sell_prices, marker='v', s=100, c='#F37021',
                   zorder=4, edgecolor='black', label='Predaj', lw=0.6)
